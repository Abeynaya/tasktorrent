...
void task_defn(Communicator& comm,
               Threadpool& tp,
               ...
               map<int2, MatrixXd>& Mat,
                map<int2, MatrixXd>& T,
               int& n_threads, int& n, int& M, int& N, int& p, int& q, const int& rank, const int& n_ranks)
{
    // Communicator comm = *commP;
    // Threadpool tp = *tpP;
    // Mapper
    ...
    if(rank == 0) printf("Starting randomized range finder on a dense matrix\n");
...
int rand_range(int n_threads, int n, int M, int N, int p, int q)
{
    ...
    // Factorize
    {
        ...
        task_defn(comm, tp, dgeqrt_tf, dtsqrt_tf, dlarfb_tf, dssrfb_tf, Mat, T, n_threads, n, M, N, p, q, rank, n_ranks);
    }
    ...
    // Gather everything on rank 0 and test for accuracy
    {
        ...
        if(rank == 0) {
            ...
            for (int j = N-1; j > -1; --j){
                ...
                int info = LAPACKE_dlarfb(LAPACK_COL_MAJOR, 'L', 'N', 'F', 'C', n, 1, n, Mat.at({j,j}).data(), n, 
                                          T.at({j,j}).data(), n, b.segment(j*n, n).data(), n);
                assert(info == 0);
