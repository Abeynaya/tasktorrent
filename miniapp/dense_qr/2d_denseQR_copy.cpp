...
int denseQR(int n_threads, int n, int M, int N, int p, int q)
{
    // MPI info
    ...
    {
        MatrixXd A = MatrixXd::NullaryExpr(M * n, N * n, gen);
    ...
    // Gather everything on rank 0 and test for accuracy
    {
        ...
        if(rank == 0) {
            cout << "Entering solve..." << b.size() << endl;
            // Test 1   
            // Q^T b
            for (int j = 0; j < N; ++j){
...
int main(int argc, char **argv)
{
    ...
    const int return_flag = denseQR(n_threads_, n_, M_, N_, p_, q_);
